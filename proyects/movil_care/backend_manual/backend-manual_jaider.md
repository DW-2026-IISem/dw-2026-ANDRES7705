# Manual de construcción — Backend NestJS (StoreLab · solo Business)

> **Objetivo:** construir desde cero, **sin ayuda de IA**, el backend de negocio de **StoreLab** (`clients`, `product-types`, `products`, `sales`) con **NestJS 12 + Sequelize** y **Clean Architecture por feature**.
>
> La construcción está dividida en **segmentos** (bloques de trabajo) que coinciden con los issues del proyecto: `ISS-01` … `ISS-08`. Cada archivo se crea con un heredoc (`cat > ... <<'EOF_BACKEND_IA'`) listo para copiar/pegar.
>
> Antes de cada archivo verás una **leyenda de capa** (🟢🟣🔵🟠⚙️) que te dice **en qué capa de la arquitectura estás** y **qué bloque de código contiene**.
...
3. [ISS-01 · Esqueleto NestJS arrancable](#3-iss-01--esqueleto-nestjs-arrancable)
4. [ISS-02 · Entorno Sequelize y common](#4-iss-02--entorno-sequelize-y-common)
5. [ISS-03 · Feature clients](#5-iss-03--feature-clients)
6. [ISS-04 · Feature product-types](#6-iss-04--feature-product-types)
7. [ISS-05 · Feature products](#7-iss-05--feature-products)
8. [ISS-06 · Sales y stock](#8-iss-06--sales-y-stock)
9. [ISS-07 · Integración business y demo](#9-iss-07--integración-business-y-demo)
...
El backend se organiza en **features** (una carpeta por entidad de negocio). Dentro de cada feature hay **4 capas** con una **regla de dependencia estricta** (las dependencias apuntan **hacia adentro**):
...
### 1.2 Leyenda de capas (cómo leer el manual)

Cada bloque de código de este manual se marca con un **color/capa**. Así sabes **en qué capa estás** y **qué contiene**:

| Marcador | Capa | Carpeta | Qué contiene | Regla de dependencia |
|---|---|---|---|---|
| 🟢 `domain` | dominio | `domain/` | entidades, puertos (interfaces), excepciones, servicios de dominio | **no depende de nadie** (sin Nest, sin Sequelize, sin HTTP) |
| 🔵 `application` | aplicación | `application/` | casos de uso (`use-cases`), DTOs, mappers | depende solo de `domain` |
...
| 🟣 `presentation` | presentación | `presentation/http/` | controladores, guards, decoradores | depende de `application` |
| ⚙️ `transversal` | transversal | `config/`, `common/`, `infrastructure/database/`, `health/`, `main.ts`, `app.module.ts` | configuración, manejo de errores, interceptores, conexión a BD | compartido por todas las features |

### 1.3 Mapa de segmentos (issues)

La construcción se divide en 8 segmentos, cada uno un **issue** trazable (`trazabilidad/ISS-XX.md`):

| Segmento | Issue | Qué se construye |
|---|---|---|
| 3 | **ISS-01** | Esqueleto NestJS arrancable (carpeta + `nest new` + ESM + tooling) |
| 4 | **ISS-02** | Entorno de configuración, `common` (excepciones/filtro/interceptores) y Sequelize |
| 5 | **ISS-03** | Primera feature `clients` (patrón completo) |
| 6 | **ISS-04** | Feature `product-types` |
| 7 | **ISS-05** | Feature `products` (FK + regla de stock) |
| 8 | **ISS-06** | Feature `sales` (transaccional) |
| 9 | **ISS-07** | Integración de negocio, seeders, pruebas y demo |
...
> ⚙️ `transversal` — envelope de respuesta, logging y timeout.
